...
#if false
                /* currently we use a static Configuration. The main reason is the missing DNS Address retrieval in Compact Framework*/
                IPAddressCollection dnsServerAddrs = DnDns.Tools.DiscoverDnsServerAddresses();

                foreach (IPAddress dnsIP_Address in dnsServerAddrs)
                {
                    s_dnsServerAddrList.Add(dnsIP_Address.ToString());
                }
#endif
...
//              enrollment_url = "http://67.202.107.163/.well-known/p2psip-enroll";
        ...
        for (int i = 0; i < iRetries; i++) {
           ...
           * (Virtual Server)
           */
          if (ReloadGlobals.HAW_Enrollment)
            enrollment_url = ReloadGlobals.EnrollmentServer;
          else
            //enrollment_url = new ReloadConfigResolve(m_ReloadConfig).ResolveConfigurationServer(ReloadGlobals.OverlayName);
